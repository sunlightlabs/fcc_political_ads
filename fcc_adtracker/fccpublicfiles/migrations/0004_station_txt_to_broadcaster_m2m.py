...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for document in document_list:
            ...
            try:
                broadcaster = orm['broadcasters.Broadcaster'].objects.get(callsign=callsign)
                document.broadcasters.add(broadcaster)
                document.save()
            except orm['broadcasters.Broadcaster'].DoesNotExist:
                pass

    def backwards(self, orm):
        document_list = orm['fccpublicfiles.PublicDocument'].objects.all()
        for document in document_list:
            callsign_list = [bc.callsign for bc in document.broadcasters.all()]
            if len(callsign_list) > 0:
                document.station = ','.join(callsign_list)
                document.save()
