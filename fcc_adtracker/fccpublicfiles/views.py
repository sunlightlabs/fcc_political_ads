...
def filing_dma_list(request, dma_id):
    ...
    filings = PoliticalBuy.nulls_last_objects.filter(dma_id=dma_id).select_related('related_FCC_file').order_by('-related_FCC_file__upload_time', '-upload_time')
...
def filing_station_list(request, callsign):
    ...
    filings = PoliticalBuy.nulls_last_objects.filter(broadcaster_callsign__iexact=callsign).select_related('related_FCC_file').order_by('-related_FCC_file__upload_time', '-upload_time')
    pagenum = request.GET.get('page', 1)
    (this_page, paginator) = paginate_filing_list(filings, pagenum)
...
def filing_state_list(request, state_id):
    ...
    if state_name:
        ...
        filings = PoliticalBuy.nulls_last_objects.filter(community_state=state_id.upper()).select_related('related_FCC_file').order_by('-related_FCC_file__upload_time', '-upload_time')
        pagenum = request.GET.get('page', 1)
        (this_page, paginator) = paginate_filing_list(filings, pagenum)
...
def fcc_most_recent(request):
    ...
    filings = PoliticalBuy.nulls_last_objects.filter(upload_time__gte=three_days_ago).select_related('related_FCC_file').order_by('-related_FCC_file__upload_time', '-upload_time')
    pagenum = request.GET.get('page', 1)
    (this_page, paginator) = paginate_filing_list(filings, pagenum)
